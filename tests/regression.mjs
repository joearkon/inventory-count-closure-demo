...
await check('follow-up work orders use a traceable full detail page', async () => {
  ...
  for (const token of ['操作日志', '新增处理记录', '基础信息', '关联单据与业务记录']) if (!`${page}\n${script}`.includes(token)) throw new Error(`missing work order detail token: ${token}`);
  for (const token of ['问题摘要', '工单原因', '执行目标', '关联研判附件', '建议动作与执行', '库存修正与验证', '系统研判过程', '业务记录', '闭环进度', '关闭门禁', '缺失证据', '处理前', '处理后', '关联业务动作', '首次库存公式', '最新库存公式', 'AI 复核意见', 'data-ai-review', '系统验证', '修正后发现新的问题', 'data-followup-work-order', 'V2 重算记录', 'data-run-page', 'D1 · 理论与实盘差异', 'S1 · 安全库存预警', 'T2 · 销入比失衡', '人工闭环确认', '升级至总部运营', 'follow_up_action']) if (!script.includes(token)) throw new Error(`missing work order workflow token: ${token}`);
  ...
  if (!/class="note-form"/.test(script) || !/\.note-form\{display:grid;gap:13px\}/.test(page) || !/\.prototype-detail-layout\{grid-template-columns:1fr\}/.test(page)) throw new Error('work order note form must have a responsive standalone layout');
