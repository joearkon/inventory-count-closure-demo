...
await check('follow-up work orders use a traceable full detail page', async () => {
  ...
  for (const token of ['问题摘要', '工单原因', '执行目标', '关联研判附件', '建议动作与执行', '库存修正与验证', '系统研判过程', '业务记录', '闭环进度', '关闭门禁', '缺失证据', '处理前', '处理后', '实际处理动作', '系统验证', '修正后发现新的问题', 'data-followup-work-order', 'V2 重算记录', 'data-run-page', 'D1 · 理论与实盘差异', 'S1 · 安全库存预警', 'T2 · 销入比失衡', '人工闭环确认', '升级至总部运营', 'follow_up_action']) if (!script.includes(token)) throw new Error(`missing work order workflow token: ${token}`);
