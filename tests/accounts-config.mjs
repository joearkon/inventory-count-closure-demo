...
async function login(email) {
  const response = await nativeFetch(`${base}/api/auth/login`, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({ email, code:'123456' }) });
  assert.equal(response.status, 200, `login failed for ${email}`);
  authCookie = (response.headers.get('set-cookie') || '').split(';')[0];
  return response.json();
}
await login('wangmin@demo.local');
...
await post('/api/accounts', {
  display_name:'QA 店员', email:qaEmail, identity_provider:'email_demo',
