...
async function r2CreateDiagnosisWorkOrder(env, anomalyId) {
  ...
  r2RefreshInventoryCases(value);
  const inventoryCase = (value.inventoryCases || []).find((item) => (item.signal_ids || []).includes(signal.id));
  const existingCaseTask = inventoryCase ? (value.operationTasks || []).find((item) => item.inventory_case_id === inventoryCase.id && item.status !== 'closed') : null;
  if (existingCaseTask) return json({ ...normalizeR2DemoState(value), operationTask: existingCaseTask, inventoryCase });
  ...
  const task = {
    ...
    source_anomaly_id: signal.id, source_rule_code: signal.rule_code, inventory_case_id:inventoryCase?.id || null,
  ...
  value.operationTasks.unshift(task); value.operationTask = task;
  r2RefreshInventoryCases(value, createdAt);
...
async function r2OperationTransition(env, taskId, body, action) {
  ...
  if (action === 'submit') {
    ...
    if (outcome === 'unresolved' || outcome === 'master_data_issue') {
      ...
      value.operationTask = task;
      r2RefreshInventoryCases(value, closedAt);
...
async function r2PrepareFourCaseShowcase(env, body = {}) {
  ...
  const expectedConfirmation = `PREPARE4:${storeCode}:${businessDate}`;
  if (storeCode !== STORE_CODE || !/^\d{4}-\d{2}-\d{2}$/.test(businessDate) || body.confirm !== expectedConfirmation) {
    return bad(`四类库存演示需要确认口令 ${expectedConfirmation}，且当前仅允许 ${STORE_CODE}。`, 409);
  }
  try {
    let value = await r2DemoState(env);
    ...
    await r2ActionResult(await r2CreateMaterialEvent(env, {
      ...
      reference:'四类库存演示 · 果糖批量到货'
    }), '建立 T2 批量收货事实');
    const countPlanCreated = await r2ActionResult(await r2CreateManualCountPlan(env, {
      store_code:storeCode, business_date:businessDate, material_names:['牛奶'], source_type:'manual',
      instruction:'四类库存演示：请复核牛奶实盘、单位、报损和收货记录。'
    }), '建立 D1 定向盘点');
    const countPlanNo = countPlanCreated.plan?.plan_no;
    if (!countPlanNo) throw new Error('D1 盘点计划未返回编号');
    await r2ActionResult(await r2CreateInitialCount(env, {
      planNo:countPlanNo, filename:`四类库存演示-牛奶盘点-${businessDate}.jpg`,
      actuals:{ '牛奶|L':80 }, recognition_note:'照片识别后由门店确认'
    }), '提交 D1 实盘结果');

    value = await r2DemoState(env);
    const activeSignals = (value.materialAnomalies || []).filter((signal) => signal.store_code === storeCode && signal.business_date === businessDate && !['closed', 'auto_closed'].includes(signal.status));
    const countVariance = activeSignals.find((signal) => signal.rule_code === 'COUNT_VARIANCE' && normalizedKey(signal.material_name) === normalizedKey('牛奶'));
    const currentSafety = activeSignals.find((signal) => signal.rule_code === 'BELOW_SAFETY_STOCK');
...
function r2ReconcileMaterialDiagnosis(value, ledger, businessDate, storeCode, sourceBatchId, updatedAt) {
  ...
  for (const row of ledger) {
    ...
    if (theoreticalQty < -negativeTolerance) {
...
function r2ReconcileAllMaterialSignalsAndCases(value, businessDate, updatedAt = now()) {
  ...
  r2RefreshInventoryCases(value, updatedAt);
  return value.materialAnomalies || [];
}

function r2InventoryRuleFamily(ruleCode) {
  if (['NEGATIVE_THEORETICAL', 'COUNT_VARIANCE'].includes(ruleCode)) return 'inventory_accuracy';
  if (ruleCode === 'BELOW_SAFETY_STOCK') return 'supply_risk';
  if (ruleCode === 'SELL_IN_IMBALANCE') return 'replenishment_balance';
  return 'other';
}

function r2InventoryRuleLabel(ruleCode) {
...
function r2InventoryObservation(signal) {
  ...
  return {
    ...
    rule_label:r2InventoryRuleLabel(signal.rule_code),
    rule_family:r2InventoryRuleFamily(signal.rule_code),
...
function r2InventoryCaseStatus(task, hasCurrentSignal, closedVerified) {
...
function r2RefreshInventoryCases(value, updatedAt = now()) {
  const latestBusinessDate = value.feishuImport?.latest_business_date || null;
  const supported = new Set(['NEGATIVE_THEORETICAL', 'COUNT_VARIANCE', 'BELOW_SAFETY_STOCK', 'SELL_IN_IMBALANCE']);
  const baseGroups = new Map();
  for (const signal of (value.materialAnomalies || []).filter((item) => supported.has(item.rule_code))) {
    const family = r2InventoryRuleFamily(signal.rule_code);
    const baseKey = `${signal.store_code}|${normalizedKey(signal.material_name)}|${signal.unit || ''}|${family}`;
  ...
  for (const [baseKey, baseSignals] of baseGroups) {
    ...
    const first = signals[0], latest = signals.at(-1), family = r2InventoryRuleFamily(latest.rule_code);
    ...
    const caseId = existing?.id || r2StableInventoryCaseId(issueKey);
    const observations = signals.map(r2InventoryObservation);
    const status = r2InventoryCaseStatus(activeTask, currentSignals.length > 0, closedVerified);
    ...
    const caseItem = {
      ...
      issue_key:issueKey,
      store_code:currentSignal.store_code,
      material_name:currentSignal.material_name,
      unit:currentSignal.unit,
      rule_family:family,
      family_label:r2InventoryFamilyLabel(family),
      current_rule_code:currentSignal.rule_code,
      current_rule_label:r2InventoryRuleLabel(currentSignal.rule_code),
      status,
      opened_business_date:first.business_date,
      latest_business_date:latest.business_date,
      first_seen_at:first.created_at || first.updated_at,
      last_seen_at:latest.updated_at || latest.created_at,
      signal_ids:[...signalIds],
      observation_ids:observations.map((item) => item.observation_id),
      observations:[...observations].reverse(),
      observation_count:observations.length,
      active_work_order_id:activeTask?.id || null,
      work_order_ids:tasks.map((task) => task.id),
      assigned_to:assignedTo,
      owner:currentSignal.owner || null,
      severity:signals.some((item) => item.severity === 'high') ? 'high' : 'mid',
      ...
      latest_evidence:currentSignal.evidence || '',
      resolution:closedTask?.resolution || null,
      closed_at:closedVerified ? closedTask.closed_at : null,
      updated_at:activeTask?.updated_at || closedTask?.updated_at || latest.updated_at || updatedAt,
      contract_version:'inventory-case-v3.0',
      recurrence_of_case_id:segmentIndex > 0 ? (existing?.recurrence_of_case_id || priorCaseId) : null,
      recurrence_index:segmentIndex
    };
...
function r2InventoryCasesView(value) {
  const cases = r2RefreshInventoryCases(value);
  const count = (status) => cases.filter((item) => status.includes(item.status)).length;
  return {
    contract_version:'inventory-case-v3.0',
    latest_business_date:value.feishuImport?.latest_business_date || null,
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (env.DEMO_STATE && request.method === 'GET' && url.pathname === '/api/inventory-cases') {
      const value = await r2DemoState(env);
      return json(r2ScopePayload(r2InventoryCasesView(value), authContext.account));
    }
    ...
    if (env.DEMO_STATE && request.method === 'POST' && url.pathname === '/api/demo-day/prepare-four-case-showcase') return r2PrepareFourCaseShowcase(env, await request.json().catch(() => ({})));
