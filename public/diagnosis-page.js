(() => {
  ...
  function shadowCard(item) {
    ...
    const traceLabels = { 'D2-TRIGGER-001':'负库存触发', 'D2-BALANCE-001':'库存公式复算', 'D2-CONTRIBUTOR-001':'负向贡献排序', 'D2-INBOUND-001':'近三日入库回溯', 'D2-COUNT-001':'有效实盘检查', 'D2-ACTION-001':'下一步动作建议' };
  ...
  async function showShadow(signalId = '') {
    ...
    try {
      ...
      panel.innerHTML = `<div class="shadow-head"><div><h2>规则 V2 对照·预览</h2><p>D2 负库存决策树 · 影子版本 ${esc(String(shadowPayload.ruleset.rule_version || '').replace('-shadow', ''))} · ${items.length} 项对照</p></div><button class="close-shadow" id="v2-preview-close">收起</button></div><div class="shadow-banner">影子研判仅用于规则验证：不建立工单、不改变库存、不替换当前 V1 结论。</div>${items.length ? items.map(shadowCard).join('') : '<div class="empty">当前没有可对照的 D2 负库存案例。</div>'}`;
