(() => {
  ...
  function issueCard(signal) {
    ...
    const previewButton = `<button class="button secondary" data-v2-preview="${esc(signal.id)}">查看 V2 推演</button>`;
  ...
  function shadowCard(item) {
    ...
    const ruleLabels = { NEGATIVE_THEORETICAL:'D2 · 负库存', COUNT_VARIANCE:'D1 · 理论与实盘差异', BELOW_SAFETY_STOCK:'S1 · 安全库存预警', SELL_IN_IMBALANCE:'T2 · 销入比失衡' };
    const strategyLabels = { verify_receipt_or_order:'核对收货、订货与调拨', verify_count_variance:'复核盘点差异', restock_or_transfer:'补货或调拨', verify_sell_in_ratio:'核对销入比' };
    const evidenceLabels = { confirmed:'已有直接证据', excluded:'已排除', suspect:'疑似，仍需核验', insufficient:'证据不足', pending_count:'等待盘点' };
    const physicalLabels = { confirmed:'已有有效实盘', excluded:'已排除实物异常', suspect:'实物状态疑似异常', insufficient:'实物证据不足', pending_count:'等待盘点' };
    const gapStateLabels = { partial:'系统已接入，历史覆盖不完整', unlinked:'有流水但未关联单据', pending:'已有单据，等待签收', not_found:'当前未找到', not_integrated:'系统尚未接入', unknown:'状态待确认' };
    const traceLabels = { 'D2-TRIGGER-001':'负库存触发', 'D2-BALANCE-001':'库存公式复算', 'D2-CONTRIBUTOR-001':'负向贡献排序', 'D2-INBOUND-001':'近三日入库回溯', 'D2-COUNT-001':'有效实盘检查', 'D2-ACTION-001':'下一步动作建议', 'D1-INPUT-001':'实盘与阈值检查', 'D1-TRIGGER-001':'盘点差异触发', 'D1-DIRECTION-001':'盘盈盘亏方向', 'S1-INPUT-001':'安全库存配置检查', 'S1-TRIGGER-001':'安全库存触发', 'S1-COUNT-001':'现场库存核验', 'T2-INPUT-001':'销入比输入检查', 'T2-TRIGGER-001':'销入比触发', 'T2-DIRECTION-001':'失衡方向判断' };
    ...
    const gaps = result.evidence_gaps || (result.missing_evidence || []).map((label) => ({ label, state:'unknown', explanation:'需要继续补充或核验' }));
    return `<article class="shadow-card" data-shadow-signal="${esc(item.signal_id)}"><h3>${esc(item.store_code)} · ${esc(item.material_name)}（${esc(item.unit)}）</h3><div class="shadow-grid"><div class="shadow-side"><h4>当前正式规则 V1</h4><p><b>${esc(ruleLabels[item.v1.rule_code] || item.v1.rule_code)}</b></p><p>${esc(item.v1.evidence || '当前规则证据待补充')}</p><p>处理策略：${esc(strategyLabels[item.v1.strategy] || '待验证')}</p></div><div class="shadow-side v2"><h4>决策树 V2 · 影子结果</h4><p><b>${esc(result.primary_location)}</b></p><p>${esc(result.primary_hypothesis)}</p><p>库存重建：${esc(formula)}</p><p>原因证据：${esc(evidenceLabels[result.cause_evidence_status] || result.cause_evidence_status)} · 实物状态：${esc(physicalLabels[result.physical_status] || result.physical_status)}</p>${gaps.length ? `<div class="evidence-gaps"><b>仍需补充或核验</b>${gaps.map((gap) => `<div class="evidence-gap"><span>${esc(gap.label)}</span><em>${esc(gapStateLabels[gap.state] || gap.state)}</em><small>${esc(gap.explanation)}</small></div>`).join('')}</div>` : '<p class="evidence-complete">当前所需证据已齐全。</p>'}<div>${result.actions.map((action) => action.route ? `<a class="action-chip" href="${esc(action.route)}">${esc(action.label)}</a>` : `<span class="action-chip">${esc(action.label)}</span>`).join('')}</div></div></div><ol class="trace">${result.decision_trace.map((step, index) => `<li><b>${index + 1}. ${esc(traceLabels[step.node_id] || '规则步骤')}</b>：${esc(step.message)}</li>`).join('')}</ol></article>`;
  }
  async function showShadow(signalId = '') {
    ...
    try {
      ...
      panel.innerHTML = `<div class="shadow-head"><div><h2>规则 V2 对照·预览</h2><p>D2 负库存、D1 盘点差异、S1 安全库存、T2 销入比 · 影子版本 ${esc(String(shadowPayload.ruleset.rule_version || '').replace('-shadow', ''))} · ${items.length} 项对照</p></div><button class="close-shadow" id="v2-preview-close">收起</button></div><div class="shadow-banner">影子研判仅用于规则验证：不建立工单、不改变库存、不替换当前 V1 结论。</div>${items.length ? items.map(shadowCard).join('') : '<div class="empty">当前没有可对照的库存异常案例。</div>'}`;
