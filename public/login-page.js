(() => {
  ...
  async function load() {
    const response = await fetch(`/api/auth/options?portal=${encodeURIComponent(portal)}`), data = await response.json(); options = data;
    ...
    const preferred = portal === 'hq' ? data.accounts.find((item) => item.role_ids.includes('hq_operations')) : data.accounts.find((item) => item.role_ids.includes('store_staff'));
    if (preferred || data.accounts[0]) select(preferred || data.accounts[0]);
  }
  by('form').onsubmit = async (event) => {
    event.preventDefault(); const button = by('submit'); button.disabled = true; by('message').textContent = '';
    try {
      const response = await fetch('/api/auth/login', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({ email:by('email').value, password:by('password').value, portal }) });
