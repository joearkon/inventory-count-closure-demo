(() => {
  ...
  const actionControl = (item, action, attrs) => action ? `<span class="plan-accepted">已采纳 · ${esc(action.owner || '待分配')}</span>${action.execution_url ? `<a class="btn secondary compact" href="${esc(action.execution_url)}">进入执行</a>` : '<span class="plan-warning">需要先建立后续工单</span>'}` : `<button class="btn secondary compact" type="button" data-accept-action="${esc(item.id)}" ${attrs}>采纳并编排</button>`;
  function ruleActionPanel(item) {
    const ids = item.recommended_action_ids || [];
    if (!ids.length) return '';
    return `<section class="detail-block unified-plan"><div class="plan-title"><div><h3>规则建议行动</h3><p>即使不运行 AI，V2 也通过同一执行计划推动问题闭环。</p></div><span>V2 ${esc(item.rule_version || '')}</span></div><div class="plan-list">${ids.map((actionId, index) => { const ref = `${item.fact_packet_id || item.id}:${actionId}`, action = planned(item, ref); return `<article><i>${index + 1}</i><div><b>${esc(ruleActionLabel[actionId] || actionId)}</b><small>来源：确定性规则 · 需要人工采纳</small></div><div class="plan-control">${actionControl(item, action, `data-action-source="rule" data-rule-action="${esc(actionId)}"`)}</div></article>`; }).join('')}</div></section>`;
  }
  ...
  function bind() {
    ...
    document.querySelectorAll('[data-accept-action]').forEach((button) => button.onclick = async () => {
      const original = button.textContent;
      button.disabled = true;
      button.textContent = '正在编排…';
      const body = button.dataset.actionSource === 'rule'
        ? { source:'rule', action_id:button.dataset.ruleAction }
        : { source:'ai', analysis_id:button.dataset.analysisId, step_index:Number(button.dataset.stepIndex) };
      try {
        const response = await fetch(`/api/inventory-cases/${encodeURIComponent(button.dataset.acceptAction)}/actions`, { method:'POST', headers:{ 'Content-Type':'application/json' }, body:JSON.stringify(body) });
        const result = await response.json();
        if (!response.ok) throw new Error(result.error || '无法采纳该建议');
        await load();
      } catch (error) {
        alert(error.message);
