(() => {
  ...
  const correctionSummaryHtml = (data) => {
    ...
    const terms = [['opening','期初'],['receipt','+ 收货'],['transfer_in','+ 调拨入'],['transfer_out','- 调拨出'],['scrap','- 报损'],['bom_consumption','- BOM 销售消耗']];
    const formulaCard = (run, title, compare) => `<article class="balance-card"><h4>${title}</h4>${terms.map(([key,label]) => { const raw = run.calculation?.quantities?.[key]?.value; const previous = compare?.calculation?.quantities?.[key]?.value; const changed = compare && raw != null && previous != null && Number(raw) !== Number(previous); return `<div class="${changed ? 'changed' : ''}"><span>${label}${changed ? ' ✓' : ''}</span><b>${qty(raw)}</b></div>`; }).join('')}<div class="balance-total"><b>理论期末</b><b>${qty(run.theoretical_closing?.value)} ${esc(unit)}</b></div></article>`;
    const rows = events.map(item => `<tr><td>${formatDate(item.created_at || item.received_at)}</td><td>${esc(({receipt:'收货',transfer_in:'调拨入',transfer_out:'调拨出',scrap:'报损'})[item.type] || item.type)}</td><td>${esc(item.operator || item.created_by || '—')}</td><td><a href="${esc(actionUrl('/flows/',data.task,item.material_name,item.business_date))}">${esc(item.document_no || item.id)}</a></td><td>${qty(item.qty)} ${esc(item.unit)}</td></tr>`).join('');
    return `<div class="correction-summary"><section><h3 class="section-title">处理前后对比</h3><div class="stock-comparison"><article><small>首次研判（${esc(first.source_signal?.business_date || '').slice(0,10)}）</small><b class="${before < 0 ? 'danger' : ''}">${qty(first.theoretical_closing?.value)} ${esc(unit)}</b><small>${esc(evidenceStatus(first.anomaly_status))}</small></article><i>→</i><article><small>最新重算（${esc(latest.source_signal?.business_date || '').slice(0,10)}）</small><b class="${latest.anomaly_status === 'not_triggered' ? 'recovered-value' : ''}">${qty(latest.theoretical_closing?.value)} ${esc(unit)}</b><small>${esc(evidenceStatus(latest.anomaly_status))}</small></article></div><div class="balance-columns">${formulaCard(first,'首次库存公式')}${formulaCard(latest,'最新库存公式',first)}</div></section><section><h3 class="section-title">关联业务动作</h3><div class="business-table-wrap"><table><thead><tr><th>时间</th><th>动作类型</th><th>操作人</th><th>单据</th><th>数量</th></tr></thead><tbody>${rows || '<tr><td colspan="5">尚无关联库存流水</td></tr>'}</tbody></table></div>${docs.length ? `<div class="document-list">${docs.map(d=>`<a class="side-link" href="${esc(documentHref(d,data.task))}">${esc(d.document_no || d.id)}</a>`).join('')}</div>` : ''}</section>${followUpHtml}<details><summary>系统验证</summary><ul class="verification-list">${checks.map(item=>`<li class="${item.ok ? 'ok' : ''}">${item.ok ? '✓' : '○'} ${item.label}</li>`).join('')}</ul></details></div>`;
  };
  ...
  const aiReviewHtml = (data) => {
    const review = data.latest_ai_analysis, caseId = data.task.inventory_case_id;
    const button = `<button class="btn primary" data-ai-review ${caseId ? '' : 'disabled'}>${review ? '重新复核' : '✧ 触发 AI 复核'}</button>`;
    if (!review) return `<section class="wo-ai-review empty-review"><b>✧ 可触发 AI 复核</b><p>${caseId ? 'AI 将基于事实包、规则轨迹和 Knowhow 给出第二意见，补充假设和缺失证据' : '当前工单未关联库存研判，关联后可进行 AI 复核'}</p>${button}<div role="status" data-ai-message></div></section>`;
    const hypotheses = review.hypotheses || [], gaps = [...new Set(hypotheses.flatMap(h => h.missing_evidence || []))];
    return `<section class="wo-ai-review"><header><b><i>AI</i> AI 复核意见</b><span>${esc(({agree:'同意规则结论',partial:'部分同意规则结论',disagree:'存在不同判断',insufficient:'证据不足'})[review.agreement] || review.agreement)}${review.stale ? ' · 历史意见，需重新复核' : ''}</span>${button}</header><div class="ai-review-body"><p class="ai-review-summary">${esc(review.summary)}</p>${hypotheses.map((h,index) => `<h4>${index ? '备选假设' : '首要假设'}</h4><article><b>${esc(h.title)}</b><small>${esc(({high:'高度疑似',medium:'有待验证',low:'证据不足'})[h.confidence] || h.confidence)} · 支持证据：${esc((h.supporting_evidence || []).join('；') || '待补充')}</small>${h.contradicting_evidence?.length ? `<p>反证：${esc(h.contradicting_evidence.join('；'))}</p>` : ''}<small>验证方法：${esc(h.falsification_test || '待补充')}</small></article>`).join('')}<h4>缺失证据（${gaps.length} 项）</h4>${gaps.length ? gaps.map(g => `<p>△ ${esc(g)}</p>`).join('') : '<p>本次未列出新增缺失项</p>'}<footer><span>模型：${esc(review.model)}</span><span>Prompt：${esc(review.prompt_version)}</span><span>事实包：${esc(review.fact_packet_id || '—')}</span><span>${formatDate(review.created_at)}</span></footer><div role="status" data-ai-message></div></div></section>`;
  };
  const render = (data) => {
    ...
    const summaryTab = `<div class="tab-section"><h3 class="section-title">闭环进度</h3>${progress}</div>${aiReviewHtml(data)}<div class="tab-section"><h3 class="section-title">关键证据</h3>${(result?.evidence_refs || []).map(ref => `<div class="key-evidence">● ${esc(typeof ref === 'string' ? ref : ref.label || ref.id)} </div>`).join('') || '<div class="empty">关键证据见下方关联研判附件。</div>'}</div><div class="tab-section"><h3 class="section-title">工单原因与执行目标</h3>${taskBriefHtml(data)}</div>`;
  ...
  const bind = (data) => {
    root.querySelector('[data-ai-review]')?.addEventListener('click', async (event) => {
      if (!data.task.inventory_case_id) return;
      const restore = setLoading(event.currentTarget, 'AI 正在复核…');
      try {
        const response = await api(`/api/inventory-cases/${encodeURIComponent(data.task.inventory_case_id)}/ai-analysis`, { method:'POST', body:'{}' });
        data.latest_ai_analysis = response.analysis;
        render(data);
      } catch (error) {
        const message = root.querySelector('[data-ai-message]');
        if (message) message.textContent = `复核失败：${error.message}，可以重试。`;
        restore();
      }
    });
