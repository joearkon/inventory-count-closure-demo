(() => {
  ...
  const problemHeroHtml = (data) => {
    ...
    return `<section class="problem-hero"><div class="hero-main"><span class="hero-kicker">${esc(result.rule_code)} · ${esc(evidenceStatus(result.anomaly_status))}</span><h2>${esc(comparison.material_name)} 当前理论库存 <strong>${theoretical == null ? '无法计算' : `${esc(theoretical)} ${esc(unit)}`}</strong></h2><p>${esc(result.primary_hypothesis)}</p><div class="hero-facts"><span>首要排查：<b>${esc(result.primary_location)}</b></span><span>实盘：<b>${packet.physical_count?.value == null ? '未取得' : `${esc(packet.physical_count.value)} ${esc(unit)}`}</b></span><span>证据缺口：<b>${(result.evidence_gaps || []).length} 项</b></span><span>已保存 V2：<b>${runs.length} 次</b></span></div></div><div class="stage-track">${stages.map((label, index) => `<div class="stage ${index + 1 < currentStage ? 'done' : index + 1 === currentStage ? 'current' : ''}"><i>${index + 1 < currentStage ? '✓' : index + 1}</i><span>${label}</span></div>`).join('')}</div></section>`;
  };
