(() => {
  ...
  const render = (data) => {
    ...
    const docs = documents.length ? documents.map((item) => `<div class="document"><div><b>${esc(item.original_filename || item.id)}</b><small>${esc(item.document_type || '业务凭证')} · ${formatDate(item.received_at)}</small></div><a class="link" href="/documents/?document=${encodeURIComponent(item.id)}">查看单据 →</a></div>`).join('') : '<div class="empty">暂无关联单据</div>';
    ...
    root.innerHTML = `<header class="page-head"><div><div class="eyebrow">跟进工单 · ${esc(task.id)}</div><h1>${esc(task.title)}</h1><p class="subtitle">${esc(task.store_code || '—')} · ${esc(typeText(task.task_type))}</p></div><span class="status ${status.css}">${status.label}</span></header>
      <div class="layout"><div class="stack">
        ...
        <section class="panel"><div class="panel-head"><h2>工单操作</h2></div><div class="panel-body"><div class="action-list">${task.status === 'pending_hq_review' ? '<button class="btn primary" id="close-task">验收并关闭</button>' : `<button class="btn" disabled>${task.status === 'closed' ? '工单已闭环' : '等待门店提交'}</button>`}<a class="btn" href="/flows/?store=${encodeURIComponent(task.store_code || '')}${material ? `&material=${encodeURIComponent(material)}` : ''}">查看库存流水</a><a class="btn" href="/count-plans/?source=work_order&work_order=${encodeURIComponent(task.id)}">下发关联盘点</a><a class="btn" href="/#operation-task-panel">返回工单列表</a></div></div></section>
      </aside></div>`;
    bind(data);
  };
