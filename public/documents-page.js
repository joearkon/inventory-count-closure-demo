(() => {
  ...
  function render() {
    ...
    by('document-rows').innerHTML = list.length ? list.map((item) => `<tr><td><b>${esc(item.no)}</b></td><td>${esc(item.date)}</td><td>${esc(item.label)}</td><td>${esc(item.flow)}</td><td>${esc(item.material)}</td><td>${esc(item.qty)} ${esc(item.unit)}</td><td><span class="tag ${mappedStatus(item.status)}">${esc(statusText(item.status))}</span></td><td>${esc(item.note || '—')}</td></tr>`).join('') : '<tr><td class="empty" colspan="8">当前筛选条件下没有单据。</td></tr>';
  }
  async function load() {
    const response = await fetch('/api/state', { cache:'no-store' }); const state = await response.json(); if (!response.ok) throw Error(state.error || '加载失败');
  ...
  ['store-filter','type-filter','status-filter'].forEach((id) => by(id).addEventListener('change', render));
