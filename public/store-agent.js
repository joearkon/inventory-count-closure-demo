(() => {
  ...
  const chat = by('store-agent-chat'), input = by('store-agent-input'), send = by('store-agent-send'), mic = by('store-agent-mic'), voiceNote = by('store-agent-voice-note');
  ...
  function showConfirm() {
    ...
    const rows = draft.intent === 'count' ? '<p>📷 需要上传盘点照片</p>' : [v.material_name && `<p>物料：<b>${esc(v.material_name)}</b></p>`, draft.intent !== 'transfer' && v.qty && `<p>数量：<b>${esc(v.qty)} ${esc(v.unit)}</b></p>`, transferRows, v.reason && `<p>原因：<b>${esc(v.reason)}</b></p>`, (draft.intent === 'scrap' || draft.intent === 'receipt') && '<p>📷 需要上传照片凭证</p>'].filter(Boolean).join('');
  ...
  async function submitSheet(event) {
    ...
    try {
      ...
      if ((draft.intent === 'scrap' || draft.intent === 'count' || draft.intent === 'receipt') && !file) throw Error('请先上传照片凭证。');
      if (draft.intent === 'transfer') {
      ...
      if (draft.intent === 'scrap') { const reason = by('agent-reason').value; const result = await post('/api/material-events', { store_code: storeCode, business_date: today(), material_name: material, unit, qty, type: 'scrap', reference: `${reason}${note ? ` · ${note}` : ''}` }); await post('/api/store-agent/evidence', { store_code: storeCode, intent: 'scrap', filename: file.name, preview_data: previewData }); addMessage(`报损已提交：${material} ${qty}${unit}，单号 ${result.materialEvents?.[0]?.document_no || '已生成'}；照片凭证已留档。`); }
      if (draft.intent === 'receipt') { const result = await post('/api/material-events', { store_code: storeCode, business_date: today(), material_name: material, unit, qty, type: 'receipt', reference: note || '门店 AI 收货登记' }); await post('/api/store-agent/evidence', { store_code: storeCode, intent: 'receipt', filename: file.name, preview_data: previewData }); addMessage(`收货已入账：${material} ${qty}${unit}，单号 ${result.materialEvents?.[0]?.document_no || '已生成'}；收货凭证已留档。`); }
  ...
  async function ask(raw) {
    ...
    try { const response = await fetch('/api/store-agent', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message, store_code: storeCode, session_id: sessionId, draft, lang: document.documentElement.lang || 'zh-CN' }) }); const data = await response.json().catch(() => ({})); waiting.remove(); if (!response.ok) throw Error(data.error || '助手暂时无法响应'); addMessage(data.reply || '已收到。'); renderTodayTasks(data.today_tasks); const intent = intentFromAction(data.action); if (intent) startDraft(intent, data.action.prefill || {}, ''); }
    catch (error) { waiting.remove(); addMessage(`暂时无法连接助手：${error.message}。`); } finally { setBusy(false); }
  }
  async function welcome() { if (welcomed) return; welcomed = true; await loadLedger(); try { const response = await fetch('/api/store-agent', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: '__welcome__', store_code: storeCode, session_id: sessionId, lang: document.documentElement.lang || 'zh-CN' }) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw Error(data.error || '读取待办失败'); addMessage(data.reply || '你好！我是你的门店运营助手。'); renderTodayTasks(data.today_tasks); } catch (_) { addMessage('你好！我是你的门店运营助手。可以帮你处理调拨、报损、收货、盘点或查询库存。'); } }
  function configureVoice() { const Recognition = window.SpeechRecognition || window.webkitSpeechRecognition; if (!Recognition) return false; recognition = new Recognition(); recognition.lang = document.documentElement.lang === 'id' ? 'id-ID' : 'zh-CN'; recognition.interimResults = true; recognition.continuous = false; let finalText = ''; recognition.onresult = (event) => { let text = ''; for (let index = event.resultIndex; index < event.results.length; index += 1) text += event.results[index][0].transcript; input.value = text; resizeInput(); if (event.results[event.results.length - 1].isFinal) finalText = text; }; recognition.onerror = () => stopVoice(false); recognition.onend = () => { const shouldSend = speaking && finalText.trim(); speaking = false; mic.classList.remove('recording'); voiceNote.classList.remove('show'); if (shouldSend) ask(finalText); }; return true; }
  function startVoice() { if (speaking || busy) return; if (!recognition && !configureVoice()) { addMessage('当前浏览器不支持语音输入，请直接输入文字。'); return; } speaking = true; mic.classList.add('recording'); voiceNote.classList.add('show'); try { recognition.start(); } catch (_) { stopVoice(false); } }
