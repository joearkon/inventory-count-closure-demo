(() => {
  ...
  function renderRecentOperations(state) {
    ...
    (state.storeTransferRequests || []).filter((item) => item.business_date === date).forEach((item) => entries.push({ at:item.received_at || item.created_at, icon:'🔄', title:item.from_store_code === storeCode ? `调出至 ${item.to_store_code}` : `调入自 ${item.from_store_code}`, meta:`营业日 ${item.business_date} · ${item.material_name} ${item.actual_received_qty ?? item.qty}${item.unit}`, status:item.status === 'received' ? '已完成' : '待收货', pending:item.status !== 'received' }));
    (state.materialEvents || []).filter((item) => item.business_date === date && ['scrap','receipt'].includes(item.type)).forEach((item) => entries.push({ at:item.created_at, icon:item.type === 'scrap' ? '🗑️' : '📦', title:item.type === 'scrap' ? `报损：${item.material_name} ${item.qty}${item.unit}` : `收货：${item.material_name} ${item.qty}${item.unit}`, meta:`营业日 ${item.business_date}${item.reference ? ` · ${item.reference}` : ''}`, status:'已完成', pending:false }));
    (state.documents || []).filter((item) => item.business_date === date && item.document_type === 'inventory_count').forEach((item) => entries.push({ at:item.received_at, icon:'📷', title:'每日盘点', meta:`营业日 ${item.business_date} · ${item.original_filename || '盘点照片已归档'}`, status:'已完成', pending:false }));
