(function () {
  ...
  function operationHqPanel(tasks, documents = []) {
    ...
    const rows = tasks.length ? tasks.slice(0, 5).map((item) => `<tr>
      ...
      <td><div class="btn-row"><button class="btn" data-show-operation-detail="${item.id}">查看详情</button>${operationTaskAction(item)}</div></td></tr>`).join('') : '<tr><td colspan="6" class="operation-empty">暂无主动运营任务。创建后会同步出现在门店待办。</td></tr>';
  ...
  function renderOperationTasks(state) {
    ...
    const selected = visible.find((task) => task.id === selectedOperationTaskId) || visible[0] || null;
    ...
    const linked = selected ? (selected.linked_document_ids || []).map((id) => (state.documents || []).find((document) => document.id === id)).filter(Boolean) : [];
    const linkedHtml = linked.length ? `<div style="margin-top:10px"><b style="font-size:13px">关联单据</b><div class="btn-row" style="margin-top:6px">${linked.map((document) => `<button class="btn" data-open-operation-document="${document.id}">${esc(document.original_filename || document.id)}</button>`).join('')}</div></div>` : '';
    const detail = selected ? `<div class="operation-detail" id="operation-detail-panel"><div class="live-eyebrow">任务详情 · ${labels[group(selected)]}</div><h3>${esc(selected.title)}</h3><p>${esc(selected.instruction)}</p><p style="margin-top:7px">${esc(selected.store_code || 'STORE001')} · 下发时间：${formatDate(selected.created_at)}${selected.proof_filename ? `<br>门店凭证：${esc(selected.proof_filename)} · 提交时间：${formatDate(selected.submitted_at)}` : ''}${selected.resolution ? `<br><span style="color:#2a9d3f">${esc(selected.resolution)} · ${formatDuration(selected.created_at, selected.closed_at)}</span>` : ''}</p>${linkedHtml}<div class="live-task-actions" style="margin-top:10px">${operationTaskAction(selected)}${selected.proof_filename ? `<button class="btn" data-open-operation-proof="${selected.id}">查看门店凭证</button>` : ''}</div></div>` : '';
    const table = visible.length ? `<div class="operation-list"><table><thead><tr><th>任务编号</th><th>任务名称</th><th>门店</th><th>状态</th><th>下发时间</th><th>操作</th></tr></thead><tbody>${visible.map((task) => `<tr><td>${esc(task.id)}</td><td>${esc(task.title)}</td><td>${esc(task.store_code || 'STORE001')}</td><td><span class="status-tag ${task.status === 'closed' ? 'closed' : task.status === 'pending_hq_review' ? 'assigned' : 'pending'}">${labels[group(task)]}</span></td><td>${formatDate(task.created_at)}</td><td><div class="btn-row">${operationTaskAction(task)}<button class="btn" data-show-operation-detail="${task.id}">查看详情</button></div></td></tr>`).join('')}</tbody></table></div>` : `<div class="operation-empty">${tasks.length ? `当前没有“${labels[selectedOperationStatus]}”状态的工单。` : '暂无跟进工单。库存或销售复盘建立工单后，会在这里统一跟踪。'}</div>`;
