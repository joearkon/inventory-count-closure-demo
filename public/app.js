(function () {
  ...
  renderAnomalyWorkspace = function(state) {
    ...
    const row = (item) => `<tr><td><b>${esc(item.case_no)}</b><small>${esc(item.opened_business_date)} 首次触发 · 最新 ${esc(item.latest_business_date)}</small></td><td><b>${esc(item.store_code)}</b><small>${Number(item.signal_count || 0)} 条物料信号</small></td><td><span class="risk-title">${item.scope === 'full_store_count' ? '多物料台账异常' : item.scope === 'count_variance_review' ? '实盘差异复核' : '日结库存风险'}</span><br><span class="severity-tag ${item.severity}">${item.severity === 'high' ? '高风险' : '中风险'}</span></td><td><span class="strategy-tag">${item.scope === 'full_store_count' ? '先治理，必要时全盘' : item.scope === 'count_variance_review' ? '物料详情 + 定向复盘' : '排除法处理'}</span><small>${esc((item.materials || []).join('、'))}</td><td class="compact-copy">${esc((item.recommended_actions || []).map((action) => action.label).join('；'))}</td><td><span class="status-tag ${item.status === 'closed' ? 'closed' : ['awaiting_store_count','targeted_count_dispatched'].includes(item.status) ? 'assigned' : 'pending'}">${esc(item.closure_mode === 'hq_forced' ? '总部强制结案' : (({open:'待总部排除',awaiting_store_count:'等待门店全盘',targeted_count_dispatched:'等待定向盘点',pending_verification:'等待总部验证',needs_hq_action:'盘点后仍有差异',pending_hq_close:'待总部结案',closed:'已结案'})[item.status] || item.status))}</span></td><td><a class="btn" href="/diagnosis-v3/?store=${encodeURIComponent(item.store_code)}">进入研判</a></td></tr>`;
