(() => {
  ...
  async function loadPlan() {
    try {
      ...
      const title = plan.plan_type === 'diagnosis_risk' ? `总部研判下发 · ${plan.material_count} 项异常物料盘点` : plan.plan_type === 'diagnosis_full' ? '总部研判下发 · 全物料盘点' : plan.plan_type === 'targeted_material' ? `总部研判下发 · ${esc(plan.target_material_name)}盘点` : '总部每日全物料盘点';
